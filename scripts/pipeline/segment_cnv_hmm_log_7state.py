        ...
        with open(bed_file) as _fh:
            for _line in _fh:
                if _line.startswith('#') or not _line.strip():
                    continue
                _ncols = len(_line.rstrip('\n').split('\t'))
                break
        _read_cols = min(8, _ncols) if _ncols >= 4 else _ncols
        df = pd.read_csv(bed_file, sep='\t', header=None, comment='#',
                         usecols=range(_read_cols))
...
def segment_chromosome(df_chrom, model=None, use_pomegranate=True):
    ...
    if use_pomegranate and model is not None and X_tensor is not None:
        try:
            with torch.no_grad():
                posteriors = model.predict_proba(X_tensor)[0].numpy()
...
def main():
    ...
    print("=" * 60)
    print(f"CopySeg {len(STATES)}-State HMM Segmentation — {mode_label}")
    print("=" * 60)
